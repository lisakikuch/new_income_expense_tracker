...
import "../styles/TransactionManager.css"; // Import new CSS file
...
const TransactionManager = () => {
    ...
    const updateTransactions = useCallback((data) => {
        ...
        const summary = data.reduce((acc, txn) => {
            const txnMonth = txn.date ? new Date(txn.date).toLocaleString("default", { month: "short" }) : "Unknown";
            if (!acc[txnMonth]) {
    ...
    useEffect(() => {
        const fetchTransactions = async () => {
            try {
                const response = await fetch(`http://localhost:5000/transactions${month ? `?month=${month}` : ""}`);
    ...
    return (
        <div className="transaction-container">
            ...
            <TransactionList transactions={transactions} loading={false} error={null} selectedMonth={month} />
